...
export default function Login() {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-6">
      <h1 className="font-display text-2xl font-bold text-white tracking-display text-center">Welcome back</h1>
      <p className="text-gray-500 text-sm text-center mt-1 mb-6">Sign in to EventSphere</p>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label className="font-mono text-[10px] uppercase tracking-widest text-rose block mb-1.5">Email</label>
          <input {...register('email')} className="w-full bg-deep border border-border rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-rose/50 transition" placeholder="you@example.com" />
        ...
        <div className="text-right">
          <Link to="/forgot-password" className="font-mono text-[10px] uppercase tracking-wider text-gray-600 hover:text-rose transition">Forgot password?</Link>
        </div>
      ...
      <p className="text-center text-sm text-gray-600 mt-6">
        ...
        <Link to="/register" className="text-rose hover:text-rose-400 transition">Sign up</Link>
      </p>
    </div>
  );
}
